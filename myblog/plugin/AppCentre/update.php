...
if(GetVars('restore','GET')!=''){
  ...
  echo '<img src="'.$zbp->host.'system/image/admin/ok.png" width="16" alt="" />';
  die();
}
  ...
  foreach ($xml->children() as $file) {
    ...
    if( ($newcrc32 == $file['crc32']) || ($newcrc32_2 == $file['crc32']) ){
      echo '<tr style="display:none;"><td><b>' . str_replace('\\','/',$file['name']) . '</b></td>';
    	$s='<img src="'.$zbp->host.'system/image/admin/ok.png" width="16" alt="" />';
    }else{
      $i+=1;
      echo '<tr><td><b>' . str_replace('\\','/',$file['name']) . '</b></td>';
    	$s='<a href="javascript:void(0)" onclick="restore(\''.base64_encode($file['name']).'\',\'file'.md5($file['name']) .'\')" class="resotrefile button" title="还原系统文件"><img src="'.$zbp->host.'system/image/admin/exclamation.png" width="16" alt=""></a>';
    }
...
function restore(f,id){
	$.get(bloghost+"feifeis/plugin/AppCentre/update.php?restore="+f, function(data){
		//alert(data);
		$('#'+id).html(data);
	});
}
