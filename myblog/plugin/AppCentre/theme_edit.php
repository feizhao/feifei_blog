...
if(GetVars('id')){
  ...
  $mt=array();
  $ft=GetFilesInDir($zbp->path . '/feifeis/theme/' . $app->id . '/','php|inc|png');
  foreach($ft as $f){
    $mt[]=filemtime($f);
  }
  $ft=GetFilesInDir($zbp->path . '/feifeis/theme/' . $app->id . '/include/','php|inc|png');
  foreach($ft as $f){
    $mt[]=filemtime($f);
  }
  $ft=GetFilesInDir($zbp->path . '/feifeis/theme/' . $app->id . '/style/','php|inc|png');
  foreach($ft as $f){
    $mt[]=filemtime($f);
  }
  $ft=GetFilesInDir($zbp->path . '/feifeis/theme/' . $app->id . '/template/','php|inc|png');
  foreach($ft as $f){
    $mt[]=filemtime($f);
  }
  $ft=GetFilesInDir($zbp->path . '/feifeis/theme/' . $app->id . '/source/','php|inc|png');
  foreach($ft as $f){
    $mt[]=filemtime($f);
  }  
  rsort($mt);
