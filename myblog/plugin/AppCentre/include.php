...
function AppCentre_AddMenu(&$m){
	global $zbp;
	$m['nav_AppCentre']=MakeLeftMenu("root","应用中心",$zbp->host . "feifeis/plugin/AppCentre/main.php","nav_AppCentre","aAppCentre",$zbp->host . "feifeis/plugin/AppCentre/images/Cube1.png");
}
...
function AppCentre_AddSiteInfoMenu(){
	...
	if($zbp->Config('AppCentre')->enabledcheck){
		...
		if( (time()-$last) > 11*60*60 ){
			echo "<script type='text/javascript'>$(document).ready(function(){  $.getScript('{$zbp->host}feifeis/plugin/AppCentre/main.php?method=checksilent&rnd='); });</script>";
...
function AppCentre_AddThemeMenu(){
	...
	echo "<script src='{$zbp->host}feifeis/plugin/AppCentre/theme.js' type='text/javascript'></script>";
}
...
function AppCentre_AddPluginMenu(){
	...
	echo "<script type='text/javascript'>var app_enabledevelop=".(int)$zbp->Config('AppCentre')->enabledevelop.";</script>";
	echo "<script type='text/javascript'>var app_username='".$zbp->Config('AppCentre')->username."';</script>";
	echo "<script src='{$zbp->host}feifeis/plugin/AppCentre/plugin.js' type='text/javascript'></script>";
}
