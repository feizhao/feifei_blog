...
function Server_Open($method){
	...
	switch ($method) {
		case 'down':
			Add_Filter_Plugin('Filter_Plugin_APP_ShowError','ScriptError',PLUGIN_EXITSIGNAL_RETURN);
		...
		case 'search':
			...
			echo str_replace('%bloghost%', $zbp->host . 'feifeis/plugin/AppCentre/main.php' ,$s);
			break;
		case 'view':
		...
		case 'checksilent':
			...
			if(strpos($s,';')!==false){
				...
				if((int)$newversion>(int)$blogversion){
					echo '$(".main").prepend("<div class=\'hint\'><p class=\'hint hint_tips\'>提示:Z-BlogPHP有新版本,请用APP应用中心插件的<a href=\'../../feifeis/plugin/AppCentre/update.php\'>“系统更新与校验”</a>升级'.$newversion.'版('. ($zbp->Config('AppCentre')->checkbeta?'Beta':'') .').</p></div>");';
				}
			}
			if($s!=0){
				echo '$(".main").prepend("<div class=\'hint\'><p class=\'hint hint_tips\'>提示:有'.$s.'个应用需要更新,请在应用中心的<a href=\'../../feifeis/plugin/AppCentre/main.php?method=check\'>“检查应用更新”</a>页升级.</p></div>");';
			}
			die();
			break;
		case 'vaild':
