...
function aotoload($class) {
	...
	} elseif (file_exists(__ROOT__ . '/include/lib/' . $class . '.php')) {
		require_once(__ROOT__ . '/include/lib/' . $class . '.php');
	} elseif (file_exists(__ROOT__ . '/include/controller/' . $class . '.php')) {
		require_once(__ROOT__ . '/include/controller/' . $class . '.php');
	} else {
		emMsg($class . '加载失败。');
	}
}
...
function checkPlugin($plugin) {
	if (is_string($plugin) && preg_match("/^[\w\-\/]+\.php$/", $plugin) && file_exists(__ROOT__ . '/content/plugins/' . $plugin)) {
		return true;
	} else {
		return false;
	}
...
function breakLog($content, $lid) {
	...
	if (!empty($ret[1])) {
		$ret[0].='<p class="readmore"><a class="am-btn am-btn-success am-btn-sm" href="' . Url::log($lid) . '">阅读全文&gt;&gt;</a></p>';
        return $ret[0];
	} elseif(Option::get('isexcerpt') == 'y') {
        return subString(trim(strip_tags($content)), 0, Option::get('excerpt_subnum')) . '<p class="readmore"><a class="am-btn am-btn-success am-btn-sm" href="' . Url::log($lid) . '">阅读全文&gt;&gt;</a></p>';
    } else {
        return $content;
    }
}
...
function emZip($orig_fname, $content) {
	...
	$zip = new ZipArchive();
	$tempzip = __ROOT__ . '/content/cache/emtemp.zip';
