 ...
 * @return bool
 */
function AblogError($errno, $errstr, $errfile, $errline) {
	...
	if(ABlogException::$isdisable==true)return true;
	
	if(ABlogException::$iswarning==false){
		if( $errno == E_WARNING )return true;
		if( $errno == E_USER_WARNING )return true;
	}
	if(ABlogException::$isstrict==false){
 ...
 * @return bool
 */
function AblogException($exception) {
	$_SERVER['_error_count'] = $_SERVER['_error_count'] +1;
	if(ABlogException::$isdisable==true)return true;
...
class ABlogException {
	...
	*/
	...
	/**
	* 获取单一实例
	* @return ABlogException
	*/
	static public function GetInstance() {
		if (!isset(self::$_zbe)) {
			self::$_zbe = new ABlogException;
		}
