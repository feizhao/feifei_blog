...
<?php shuffle($tag_cache);$tag_cache = array_shift(array_chunk($tag_cache,40));shuffle($tag_cache);foreach($tag_cache as $value): $color = dechex(rand(0,16777215));?><a style="color:#<?php echo $color;?>" href="<?php echo Url::tag($value['tagurl']); ?>" title="相关内容<?php echo $value['usenum']; ?> 篇"><?php echo $value['tagname']; ?></a>
<?php endforeach; ?>
<?php }?>
<?php
//widget：分类
...
function widget_newcomm($title){
	...
	?>
	<li id="comment"><a href="<?php echo $url; ?>"><?php echo $value['content']; ?></a></li>
	<?php endforeach; ?>
	</ul>
<?php }?>
<?php
...
function blog_navi(){
	...
	foreach($navi_cache as $value):
		...
		<li class="<?php echo $current_tab;?>">
                <a href="<?php echo $value['url']; ?>" <?php echo $newtab;?>><span><?php echo $value['naviname']; ?></span></a>
                <?php if (!empty($value['children'])) :?>
                <ul>
...
function blog_sort($blogid){
	...
	?>
	<?php if(!empty($log_cache_sort[$blogid])): ?><a href="<?php echo Url::sort($log_cache_sort[$blogid]['id']); ?>"><?php echo $log_cache_sort[$blogid]['name']; ?></a>
	<?php endif;?>
<?php }?>
<?php
//blog：文章标签
...
function neighbor_log($neighborLog){
	extract($neighborLog);?>
	<?php if($prevLog):?>
	<p><b>&lt;&lt;</b><a href="<?php echo Url::log($prevLog['gid']) ?>"><?php echo $prevLog['title'];?></a></p>
	<?php endif;?>
	<?php if($nextLog && $prevLog):?>
	<?php endif;?>
	<?php if($nextLog):?>
	<p class="rt"><a href="<?php echo Url::log($nextLog['gid']) ?>"><?php echo $nextLog['title'];?></a><b>&gt;&gt;</b></p>
	<?php endif;?>
<?php }?>
<?php
//blog：评论列表
	...
	$go = $db->query($sql);
		while($row = $db->fetch_array($go)){
			...
			if(picthumb($row['gid'])){
				...
				$img_url = TEMPLATE_URL.'images/random/pic ('.rand(1,20).').jpg';
			}?>
					<li><a href="<?php echo Url::log($row['gid']); ?>"><img src="<?php echo $img_url; ?>" /><h4><?php echo $row['title']; ?></h4></a></li>
<?php		}
}
?>
<?php
//newpost
function indexnewlist($way="newpost",$getnum="8") {
    $order = 'ORDER BY date DESC';
    $db=MySql::getInstance();
    $logs = $db->query("SELECT gid,title,date,comnum FROM " . DB_PREFIX . "blog WHERE hide='n' and type='blog' $order LIMIT 0, $getnum");
    while ($row = $db->fetch_array($logs)){
    $row['title'] = htmlspecialchars($row['title']);
     ?>
    <li><a href="<?php echo Url::log($row['gid']); ?>"><?php echo $row['title']; ?></a><span><?php echo gmdate('m-d', $row['date']); ?></span></li>
	<?php } ?>
<?php }?>
<?php
//getlist
...
$db = MySql::getInstance();
$sql2 = "SELECT gid,title,date FROM ".DB_PREFIX."blog WHERE sortid=".$sort."  AND hide='n' ORDER BY `date` DESC LIMIT 8";
$list = $db->query($sql2);
while($row = $db->fetch_array($list)){
$row['date'] += $timezone * 3600;
?>
<li><a href="<?php echo Url::log($row['gid']);?>" title="<?php echo $row['title'];?>"><?php echo $row['title'];?></a><span class="updatetime"><?php echo gmdate('m-d', $row['date']);?></span></li>	
	<?php }?>
<?php } ?>
<?php
//slider
function home_slide(){
			...
			$imgs = $db->query($sql);
    		while($row = $db->fetch_array($imgs)){
          $slide .= '<li><a href="'.Url::log($row['g']).'" target="_blank"><img src="'.BLOG_URL.substr($row['filepath'],3,strlen($row['filepath'])).'" alt="'.$row['t'].'" /></a><h3>'.$row['t'].'</h3>'; }
          echo $slide;		
	}
?>